...
type CanonMessage = { type: "message"; role: "user"; content: Array<{ type: "input_text"; text: string } | { type: "input_image"; image_url: string }>; };
...
export function ensureSessionId(
  ...
  const sid = typeof (crypto as any).randomUUID === "function" ? (crypto as any).randomUUID() : `${Date.now()}-${Math.random().toString(16).slice(2)}`;
  remember(key, sid);
  return sid;
}
